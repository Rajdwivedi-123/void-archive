...
export function createFacilityProgress(): FacilityProgress {
  return {
    version: 6, epoch: 0, location: "nexus", pose: facilityPoses.nexus, discoveredRooms: ["nexus"], unlockedShortcuts: [], completedInteractions: [], n07Clues: [], recordSearches: [], signalResult: null,
...
export function loadFacilityProgress(): FacilityProgress {
  ...
  try {
    const saved = JSON.parse(localStorage.getItem(N07_LEVEL_KEY) ?? localStorage.getItem(CONSEQUENCE_KEY) ?? localStorage.getItem(INVESTIGATION_KEY) ?? localStorage.getItem(PHASE15_LEGACY_KEY) ?? localStorage.getItem(FACILITY_KEY) ?? "null") as Partial<FacilityProgress> | null;
    const savedVersion = (saved as { version?: number } | null)?.version;
    if (!saved || (savedVersion !== 2 && savedVersion !== 3 && savedVersion !== 4 && savedVersion !== 5 && savedVersion !== 6)) {
    ...
    return {
      ...fallback, ...saved, version: 6, epoch: Number.isFinite(saved.epoch) ? saved.epoch as number : 0, location, pose: safeFacilityPose(location, saved.pose),
...
export function saveFacilityProgress(progress: FacilityProgress, replace = false) {
  try {
    const stored = JSON.parse(localStorage.getItem(N07_LEVEL_KEY) ?? "null") as Partial<FacilityProgress> | null;
    ...
    localStorage.setItem(N07_LEVEL_KEY, JSON.stringify({ ...progress, version: 6, epoch: incomingEpoch, pose: safeFacilityPose(progress.location, progress.pose), consequences: sanitizeConsequences(progress.consequences), n07: sanitizeN07Level(progress.n07, progress.consequences.endingCommit?.vector ?? "adaptive") }));
  } catch { /* optional persistence */ }
}

export function clearFacilityProgress() { try { localStorage.removeItem(N07_LEVEL_KEY); localStorage.removeItem(CONSEQUENCE_KEY); localStorage.removeItem(INVESTIGATION_KEY); localStorage.removeItem(PHASE15_LEGACY_KEY); localStorage.removeItem(FACILITY_KEY); } catch { /* optional persistence */ } }
