...
export function createFacilityProgress(): FacilityProgress {
  return {
    version: 5, epoch: 0, location: "nexus", pose: facilityPoses.nexus, discoveredRooms: ["nexus"], unlockedShortcuts: [], completedInteractions: [], n07Clues: [], recordSearches: [], signalResult: null,
...
export function loadFacilityProgress(): FacilityProgress {
  ...
  try {
    const saved = JSON.parse(localStorage.getItem(CONSEQUENCE_KEY) ?? localStorage.getItem(INVESTIGATION_KEY) ?? localStorage.getItem(PHASE15_LEGACY_KEY) ?? localStorage.getItem(FACILITY_KEY) ?? "null") as Partial<FacilityProgress> | null;
    const savedVersion = (saved as { version?: number } | null)?.version;
    if (!saved || (savedVersion !== 2 && savedVersion !== 3 && savedVersion !== 4 && savedVersion !== 5)) {
    ...
    return {
      ...fallback, ...saved, version: 5, epoch: Number.isFinite(saved.epoch) ? saved.epoch as number : 0, location, pose: safeFacilityPose(location, saved.pose),
...
export function saveFacilityProgress(progress: FacilityProgress, replace = false) {
  try {
    const stored = JSON.parse(localStorage.getItem(CONSEQUENCE_KEY) ?? "null") as Partial<FacilityProgress> | null;
    ...
    localStorage.setItem(CONSEQUENCE_KEY, JSON.stringify({ ...progress, version: 5, epoch: incomingEpoch, pose: safeFacilityPose(progress.location, progress.pose), consequences: sanitizeConsequences(progress.consequences) }));
  } catch { /* optional persistence */ }
}

export function clearFacilityProgress() { try { localStorage.removeItem(CONSEQUENCE_KEY); localStorage.removeItem(INVESTIGATION_KEY); localStorage.removeItem(PHASE15_LEGACY_KEY); localStorage.removeItem(FACILITY_KEY); } catch { /* optional persistence */ } }
