...
export function ArchiveHUD({ active, stage }: ArchiveHUDProps) {
  ...
  const approachingThree = stage === "object-two-departure" || stage === "measurement-passage" || objectThreeActive;
  const approachingTwo = (stage === "deep-archive" || objectTwoActive) && !approachingThree;
  const inTransit = ["departure", "exit", "corridor", "deep-archive", "object-two-departure", "measurement-passage"].includes(stage);
  return (
        ...
        <div className="text-right">
          ...
          <div className="mt-2 flex items-center gap-1.5 text-[8px] tracking-[0.16em] sm:gap-2 sm:tracking-[0.2em]">
            {[1, 2, 3, 4, 5, 6].map((index) => (
              <span key={index} className={index === 1 && !approachingTwo && !approachingThree ? "text-white" : index === 2 && approachingTwo ? "text-white" : index === 3 && approachingThree ? "text-white" : "text-white/22"}>0{index}</span>
            ))}
          </div>
        </div>
      </div>
      <div className="flex items-end justify-between gap-4">
        <div className="max-w-[13rem] border-l border-white/15 pl-3">
          <p className="flex items-center gap-2 font-semibold text-white/82">
            ...
            {inTransit ? "ARCHIVE LINK / ACTIVE" : objectThreeActive ? `${temporalRingArtifact.containment} / ACTIVE` : objectTwoActive ? `${liquidMirrorArtifact.containment} / ACTIVE` : `CONTAINMENT / ${active ? "ACTIVE" : "STANDBY"}`}
          </p>
          <p className="mt-2 text-[9px] leading-5 tracking-[0.26em] text-white/38">{inTransit ? approachingThree ? "DEPTH 03  /  MEASUREMENT PASSAGE" : "DEPTH 02  /  ACCESS RESTRICTED" : objectThreeActive ? "SECTOR T-03  /  CHRONOLOGY LIVE" : objectTwoActive ? "SECTOR M-02  /  OBSERVATION LIVE" : `SECTOR G-01  /  FIELD LOCK ${active ? "NOMINAL" : "PENDING"}`}</p>
        </div>
        <div className="text-right text-white/44">
          <p>{stage === "object-two-inspection" || stage === "object-three-inspection" ? "HOLD" : "SCROLL"}</p>
          <p className="mt-2 text-xs">{objectThreeActive ? "03" : objectTwoActive ? "02" : "↓"}</p>
        </div>
      </div>
    </div>
  );
